...
function ttx_benchmark_calc($bench_date) {
    ...
    <h6 class="center">All detections on <?php echo ctx_real_datestr($bench_date); ?> including archive, unilats & NO META (Global)</h6>
    <div class="center" id="regions_div" style="width: 900px; height: 500px; display: table;  margin: auto;"></div>
    <h6 class="center">All detections on <?php echo ctx_real_datestr($bench_date); ?> including archive, unilats & NO META (Europe)</h6>
...
function ttx_top_stories_month($p_date,$ttx_type,$ttx_filter,$ttx_limit) {
    ...
    elseif($ttx_type=='year') {
        if ($ttx_filter == "PARTNERS") { $pageparam="11";} else {$pageparam="13";} // switch year out
        echo "<div align='center'>
        <a href='index.php?tb=$pageparam&dt=", (substr(date('Y-m-d', strtotime("-1 year", strtotime($p_date))), 0, 5) . "01-01"), "' class='waves-effect waves-light btn'>", date('Y', strtotime("-1 year", strtotime($p_date))), "</a>
        <a href='index.php?tb=$pageparam&dt=", (substr(date('Y-m-d', strtotime("-0 month", strtotime($p_date))), 0, 5) . "01-01"), "' class='waves-effect waves-light btn blue'>", date('Y', strtotime("0 year", strtotime($p_date))), "</a>";
        if (substr($p_date, 0, 5)!= substr($today, 0, 5)) {
            echo " <a href='index.php?tb=$pageparam&dt=", (substr(date('Y-m-d', strtotime("-0 month", strtotime($today))), 0, 5) . "01-01"), "' class='waves-effect waves-light btn grey'>", date('Y', strtotime("-0 month", strtotime($today))), "</a>";
        }
         echo "</div>";
        $bench_start_date = substr($p_date,0,5)."01-01" ;
